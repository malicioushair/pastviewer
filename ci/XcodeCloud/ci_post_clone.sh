...
install_qt() {
    ...
    QT_MODULES="qtdeclarative qtmultimedia qtlocation qtpositioning qtshadertools qttools"

    python3 -m aqt install-qt mac desktop "$QT_VERSION" -O "$HOME/Qt" -m $QT_MODULES
    python3 -m aqt install-qt mac ios "$QT_VERSION" -O "$HOME/Qt" -m $QT_MODULES
